...
// Jika keywordnya kosong, maka return semua data movies yang ada
function GetMovies(keyword="") {
  //looping array of obj movies
  let result = []
  let film = String(keyword).toUpperCase().trim();
  
  movies.forEach((listMovie) => {
    let isMatched = SearchMovies(listMovie.title, film)
    if(isMatched) {
      //masukin data yang matched ke variable result
      result.push(listMovie)
    }
  })
  return result
}
...
function SearchMovies(title = "", keyword = "") {
  ...
  const keywordMovie = String(keyword).toUpperCase().trim();

  return movieTitle.includes(keywordMovie)
}

function ShowMovies(keyword="") {
  // panggil function getMovies
  const content = GetMovies(keyword)
  let contentMovies = ""
  
  // panggil element html dengan id="list-movies"
  const elListData = document.getElementById("list-movies")
    ...
    contentMovies += `
      <div class="col">
        <div class="card">
          ...
          <div class="card-body">
            <h2 class="text-center card-title">${element.original_title}</h2>
            <h6 class="text-center card-title"> Released ${element.release_date} ‧ Rate ${element.vote_average}</h6>
            <p class="text-center card-text">${element.overview}</p>
          </div>
        </div>
      </div>
    `
  ...
  elListData.innerHTML = contentMovies
  return content
}
...
function OnSearch(event) {
  // untuk handle behavior onclick yg defaultnya reload supaya nggak reload
  event.preventDefault()
  // untuk dapetin value dri input
  const input = document.getElementById("input-search").value
  
  // tampung element error
  let error = document.getElementById("error-not-found");
  
  // panggil function showMovies
  const result = ShowMovies(input)
  
  //cek kalo movie yg dicari nggak ada, tampilin element html id="error-not-found"
  if(result.length === 0) {
